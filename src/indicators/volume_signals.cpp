 ...
 */
CMFSignal::CMFSignal(int period = 20, double bullish_threshold = 0.1, double bearish_threshold = -0.1, int offset = 0) : Indicator("Chaikin Money Flow Signal", "cmf-signal-" + std::to_string(period) + "-" + std::to_string(bullish_threshold) + "-" + std::to_string(bearish_threshold) + "-" + std::to_string(offset), offset, {-1, 1}), period(period), bullish_threshold(bullish_threshold), bearish_threshold(bearish_threshold) {}
