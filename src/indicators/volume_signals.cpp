 ...
 */
CMFSignal::CMFSignal(int period, double bullish_threshold, double bearish_threshold, int offset) : Indicator("Chaikin Money Flow Signal", "cmf-signal-" + std::to_string(period) + "-" + std::to_string(bullish_threshold) + "-" + std::to_string(bearish_threshold) + "-" + std::to_string(offset), offset, {-1, 1}), period(period), bullish_threshold(bullish_threshold), bearish_threshold(bearish_threshold) {}
